...
const BIKE_SELECT = `SELECT bikes.id, bikes.code, bikes.public_code, bikes.station_id, stations.name station_name, bikes.status, bikes.battery_level,
  bikes.model, bikes.serial_number, bikes.last_service_at, bikes.retired_at, bikes.created_at, bikes.updated_at,
...
async function maintenanceStart(request,DB,json,actor,context,bikeId,readJson){const body=await readJson(request);const reason=text(body?.reason,500,5);const incidentId=body?.incidentId?integer(body.incidentId):null;const assignee=body?.assignedToUserId?integer(body.assignedToUserId):null;if(!reason)return invalid(json);
  ...
  const result=await DB.batch([DB.prepare(`INSERT INTO maintenance_records (bike_id,incident_id,opened_by_user_id,assigned_to_user_id,status,reason,started_at) VALUES (?,?,?,?, 'in_progress',?,CURRENT_TIMESTAMP)`).bind(bikeId,incidentId,actor.id,assignee,reason),DB.prepare("UPDATE bikes SET status='maintenance',updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(bikeId),DB.prepare("UPDATE bike_incidents SET status='in_progress',assigned_to_user_id=COALESCE(?,assigned_to_user_id),updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(assignee,incidentId||0)]);
  const recordId=result[0].meta.last_row_id;await audit(DB,actor,context,'maintenance.open','maintenance',recordId,{bikeId,incidentId});return json({success:true,id:recordId},201);}
...
async function notificationsCreate(request,DB,json,actor,context,readJson){const body=await readJson(request);const title=text(body?.title,120,2);const message=text(body?.body,1000,2);const type=code(body?.type||'service',40);const locale=String(body?.locale||'all');const target=String(body?.target||'active');const ids=Array.isArray(body?.userIds)?[...new Set(body.userIds.map(integer).filter(Boolean))].slice(0,100):[];if(!title||!message||!type||!['all','fr','en','es','pt','ar'].includes(locale)||!['active','all','users'].includes(target)||(target==='users'&&!ids.length))return invalid(json);
